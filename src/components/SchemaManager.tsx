...
import { Card, Table, Button, Space, Popconfirm, message, Modal, Form, Tooltip, Drawer } from 'antd';
import { DragDropContext, Droppable, Draggable } from '@hello-pangea/dnd';
import { collection, getDocs, deleteDoc, doc, setDoc, updateDoc } from 'firebase/firestore';
import { db } from '../firebase';
import { Plus, Edit, Trash2, Settings2, ChevronDown, ChevronUp, Columns, Save, GripVertical } from 'lucide-react';
...
export default function SchemaManager() {
  ...
  const handleSave = async () => {
    try {
      ...
      if (parsedSchema.type !== 'object' || !parsedSchema.properties) {
        message.error('מבנה סכמה לא תקין. הסכמה חייבת להיות מסוג "object" עם מאפיין "properties".');
        return;
      }
  ...
  const openColumnDrawer = (schemaId: string, columnName?: string) => {
    ...
    if (columnName) {
      ...
      columnForm.setFieldsValue({
        ...
        ...(componentType === 'dropdown' && columnData.dropdownOptions && {
          dropdownOptionsType: columnData.dropdownOptions.sourceTable ? 'dynamic' : 'static',
  ...
  const saveColumn = async () => {
    try {
      ...
      if (oldColumnName && oldColumnName !== newColumnName) {
        ...
        Object.entries(updatedSchema.schema.properties).forEach(([key, value]) => {
        ...
        if (updatedSchema.schema.required?.includes(oldColumnName)) {
          updatedSchema.schema.required = updatedSchema.schema.required.filter(name => name !== oldColumnName);
          if (values.required) {
            updatedSchema.schema.required.push(newColumnName);
          }
        } else if (values.required) {
          updatedSchema.schema.required = [...(updatedSchema.schema.required || []), newColumnName];
        }
        
        if (updatedSchema.uiSchema) {
          if (updatedSchema.uiSchema["ui:order"]) {
            updatedSchema.uiSchema["ui:order"] = updatedSchema.uiSchema["ui:order"].map((item: string) => 
              item === oldColumnName ? newColumnName : item
            );
          }
          
          if (updatedSchema.uiSchema[oldColumnName]) {
            updatedSchema.uiSchema[newColumnName] = updatedSchema.uiSchema[oldColumnName];
        ...
        const isCurrentlyRequired = updatedSchema.schema.required?.includes(newColumnName) || false;
        if (values.required && !isCurrentlyRequired) {
          updatedSchema.schema.required = [...(updatedSchema.schema.required || []), newColumnName];
        } else if (!values.required && isCurrentlyRequired) {
          updatedSchema.schema.required = updatedSchema.schema.required.filter(name => name !== newColumnName);
        }
      }
      ...
      message.success(`העמודה ${newColumnName} ${editingColumn ? 'עודכנה' : 'נוספה'} בהצלחה`);
  ...
  const deleteColumn = async (schemaId: string, columnName: string) => {
    try {
      ...
      if (updatedSchema.schema.required?.includes(columnName)) {
        updatedSchema.schema.required = updatedSchema.schema.required.filter(name => name !== columnName);
      }
      
      if (updatedSchema.uiSchema) {
        if (updatedSchema.uiSchema["ui:order"]) {
          updatedSchema.uiSchema["ui:order"] = updatedSchema.uiSchema["ui:order"].filter((item: string) => item !== columnName);
        }
  ...
  const expandableConfig = {
    ...
    expandedRowRender: (record: SchemaConfig) => {
      const columnsData = Object.entries(record.schema.properties).map(([key, prop]) => {
      ...
      const nestedColumns = [
        ...
        },
        {
          ...
          key: 'addable',
          width: 80,
          render: (_: any, record: any) => record.addable === false ? 'לא' : 'כן',
        },
        {
          title: 'פעולות',
          key: 'actions',
      ...
      return (
        <DragDropContext
          onDragEnd={async (result) => {
            ...
            try {
              await updateDoc(doc(db, 'schemas', record.id), cleanForFirestore(updatedSchema));
        ...
        >
          <div className="mr-12 mb-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-medium">עמודות של {record.title || record.tableName}</h3>
              <Button 
                type="primary" 
                        ...
                        >
                          {(provided, snapshot) => (
                            <tr
                              ...
                              className={`border-b ${snapshot.isDragging ? 'bg-blue-50' : 'bg-white'}`}
                            >
                              ...
                              <td className="px-4 py-2">
                                {item.format ? `${item.type} (${item.format})` : item.type}
                              </td>
                              <td className="px-4 py-2">{item.componentType}</td>
                              <td className="px-4 py-2">{item.required ? 'כן' : 'לא'}</td>
                              <td className="px-4 py-2">{item.tableOptions?.width || '-'}</td>
                              <td className="px-4 py-2">{item.tableOptions?.sortable ? 'כן' : 'לא'}</td>
                              <td className="px-4 py-2">{item.tableOptions?.filterable ? 'כן' : 'לא'}</td>
                              <td className="px-4 py-2">{item.hidden ? 'כן' : 'לא'}</td>
                              <td className="px-4 py-2">{item.addable !== false ? 'כן' : 'לא'}</td>
                              <td className="px-4 py-2">
                                <Space>
                                  <Button
                                    type="text"
                                    icon={<Edit className="w-4 h-4" />}
                                    onClick={() => openColumnDrawer(record.id, item.name)}
                                  />
                                  <Popconfirm
                                    title={`האם אתה בטוח שברצונך למחוק את העמודה ${item.name}?`}
                                    onConfirm={() => deleteColumn(record.id, item.name)}
                                  >
                                    <Button
                                      type="text"
                                      danger
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'updatedAt',
      key: 'updatedAt',
      render: (text: string) => text ? new Date(text).toLocaleString('he-IL') : '-',
    },
    {
      title: 'פעולות',
      key: 'actions',
        ...
        <Card>
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-semibold">ניהול סכמות</h1>
            </div>
            <Space>
              <Button
                type="primary"
